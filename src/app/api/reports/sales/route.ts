...
export async function GET(request: NextRequest) {
  try {
    ...
    const paymentMethods = {
      cash: orders.filter((o: any) => o.paymentMethod === 'cash').reduce((sum: number, o: any) => sum + o.totalAmount, 0),
      card: orders.filter((o: any) => o.paymentMethod === 'card').reduce((sum: number, o: any) => sum + o.totalAmount, 0),
    };
...
function groupSalesByPeriod(orders: any[], period: string): any[] {
  ...
  orders.forEach((order) => {
    ...
    if (order.paymentMethod === 'cash') grouped[key].paymentMethods.cash += order.totalAmount;
    else if (order.paymentMethod === 'card') grouped[key].paymentMethods.card += order.totalAmount;
  });
